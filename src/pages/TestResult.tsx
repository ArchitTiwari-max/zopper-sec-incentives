    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-gray-50 p-4 py-12">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <button
            onClick={() => {
              // Construct a submission object compatible with TestDetails
              const submission = {
                secId: result.secDetails?.secId || result.phone,
                phone: result.phone,
                name: result.secDetails?.name,
                sessionToken: 'review-mode',
                responses: result.responses,
                score: result.score,
                totalQuestions: result.totalQuestions,
                submittedAt: result.submittedAt,
                completionTime: result.completionTime,
                storeName: result.secDetails?.store?.storeName,
                storeCity: result.secDetails?.store?.city
              }
              navigate('/test-details', { state: { submission } })
            }}
            className="px-8 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 font-semibold shadow-lg hover:shadow-xl transition-all"
          >
            👁️ Review Answers
          </button>
          <button
