...
export function AdminProctoringAlerts() {
  ...
  const load = async () => {
    const url = phone ? `${config.apiUrl}/proctoring/events?phone=${encodeURIComponent(phone)}` : `${config.apiUrl}/proctoring/events`
    const res = await fetch(url)
    const j = await res.json()
    setEvents(j.data || [])
    if (phone) {
      const s = await fetch(`${config.apiUrl}/proctoring/score?phone=${encodeURIComponent(phone)}`)
  ...
  const grouped = useMemo(() => {
    ...
    // Filter out mic_active and snapshot events as they are not violations
    const filteredEvents = events.filter(e => e.eventType !== 'mic_active' && e.eventType !== 'snapshot')
    for (const e of filteredEvents) {
      const k = e.phone || e.secId
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <input className="border rounded px-3 py-2" placeholder="Filter by Phone Number" value={phone} onChange={e => setPhone(e.target.value)} />
          ...
          {grouped.map(([k, list]) => (
            <div key={k} className="bg-white rounded shadow p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="font-semibold">Phone: {k}</div>
