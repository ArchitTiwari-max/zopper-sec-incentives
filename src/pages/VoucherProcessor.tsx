...
import { FaArrowLeft, FaUpload, FaSpinner, FaCheckCircle, FaExclamationTriangle, FaTimesCircle, FaDownload, FaFileExcel } from 'react-icons/fa'
...
export function VoucherProcessor() {
  const { auth } = useAuth()
...
const response = await authFetch(`${config.apiUrl}/admin/process-voucher-excel`, {
        method: 'POST',
        headers: {
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">📋 Voucher Code Processor</h1>
            <p className="text-gray-600 text-sm">
              Upload Excel file with voucher codes to automatically update sales reports and mark them as paid.
            </p>
          </div>
        </div>
        ...
        {!results ? (
          <div className="bg-white rounded-2xl shadow-md p-6">
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-xl">
              ...
              <ol className="text-sm text-blue-800 space-y-1 ml-4 list-decimal">
                <li>Export Excel file from Admin Dashboard</li>
                <li>Add voucher codes in the "Voucher Code" column for records you want to pay</li>
                <li>Only records with "Pending" payment status and non-empty voucher codes will be processed</li>
          ...
          <AnimatePresence>
            ...
            >
                    ...
                    <tbody>
                      {results.logs.map((log, index) => (
                        <tr key={index} className="border-t hover:bg-gray-50">
                          ...
                          <td className="p-3 font-mono text-xs">{log.reportId}</td>
                          <td className="p-3">
              ...
              {/* Detailed Summary */}
              {results.summary.updatedReports.length > 0 && (
                <div className="bg-white rounded-2xl shadow-md p-6">
                  <h3 className="text-lg font-semibold mb-4 text-green-700">
                    ✅ Successfully Updated Reports ({results.summary.updatedReports.length})
                  </h3>
                  <div className="max-h-64 overflow-y-auto">
                    <table className="w-full text-sm">
                      <thead className="bg-green-50 sticky top-0">
                        <tr>
                          <th className="text-left p-3">Report ID</th>
                          <th className="text-left p-3">SEC User</th>
                          <th className="text-left p-3">Store</th>
                          <th className="text-left p-3">Device</th>
                          <th className="text-left p-3">Incentive</th>
                          <th className="text-left p-3">Voucher Code</th>
                        </tr>
                      </thead>
                      <tbody>
                        {results.summary.updatedReports.map((report, index) => (
                          <tr key={index} className="border-t hover:bg-green-50">
                            <td className="p-3 font-mono text-xs">{report.reportId}</td>
                            <td className="p-3">{report.secUser?.secId || report.secUser?.phone}</td>
                            <td className="p-3">{report.store?.storeName}</td>
                            <td className="p-3">{report.device?.ModelName}</td>
                            <td className="p-3">₹{report.incentiveEarned}</td>
                            <td className="p-3">
                              <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded font-mono text-xs">
                                {report.voucherCode}
                              </span>
